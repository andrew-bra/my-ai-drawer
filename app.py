...
def get_random_prompt_from_cloud(api_key):
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "请随机生成一条极具画面感、高质量的AI绘图提示词。
【严厉要求】：
1. 每次必须从以下元素中**随机抽取1-2个元素**作为主体，绝对不能重复：
   - 动物类（如：猫咪、哈士奇、机械战狼、独角兽、远古巨兽、深海鲸鱼）
   - 人物类（如：赛博朋克黑客、中世纪骑士、古风刺客、宇航员、未来机甲少女）
   - 场景静物类（如：废土世界、天空之城、微距水滴、深邃星空、蒸汽朋克工厂、老电影一角）
2. 风格要在以下类型中随机：科幻、奇幻、国风水墨、复古胶片、超现实主义、写实摄影。
3. 必须包含画面细节、光影效果（如丁达尔效应、逆光、霓虹反光）。
4. 直接输出提示词内容，绝对不要有任何废话解释，不要带引号，控制在60个字以内，必须是中文。"}
        ],
        "temperature": 0.9
    }
    try:
